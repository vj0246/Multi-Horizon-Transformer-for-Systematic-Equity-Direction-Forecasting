...
- `Source/Ingestion/quotes.py` — LIVE quote routing (execution-side; **never feeds a backtest**). NSE first for spot + delivery %, yfinance for the tradeable price because yfinance `history()` returns only SETTLED daily bars, so mid-session its last close is yesterday's. No synthetic fallback: raises if both sources fail, since a fabricated price that looks real is worse than an error. `--indices` prints NIFTY/SENSEX/BANKNIFTY/VIX.
...
- `Source/Ingestion/screener.py` — **the first fundamentals source here with real HISTORY**: ~11 years of annual P&L, ~13 quarters of results, ~12 quarters of shareholding (promoter/FII/DII). Unlike nse.py (today's values only) this IS usable as features - but only through `point_in_time()`. **Every row carries `available_from` = period_end + the SEBI LODR disclosure lag (quarterly +45d, annual +60d, shareholding +21d), taken at the limit.** A quarter ENDING 31 Mar is not ANNOUNCED until ~15 May; filtering on the period label instead of `available_from` hands the model results the market had not seen and is the easiest way to fake alpha from fundamentals. Never filter on `period`. Does NOT fix survivorship - Screener only publishes currently-listed names.
- `Source/Ingestion/nse.py` — authoritative NSE India fundamentals (P/E, sector P/E, market cap, 52w range, annualised vol, and `delivery_pct` which yfinance does NOT expose - the share of volume actually settled vs squared off intraday, a real conviction/microstructure signal). Serves only TODAY's values, so these are look-ahead leakage if applied backwards. The point of the module is `append_archive`: it APPENDS a dated snapshot per run, building a genuine point-in-time panel going forward and finally removing the "no free point-in-time NSE fundamentals" blocker. `archive_span()['usable_as_features']` stays False until >=250 distinct dates - do not use these as model features before that. Needs `curl_cffi` (chrome impersonation); NSE blocks datacenter IPs so it fails on CI and works from an Indian connection. Idempotent per day; never rewrites a previous day's row.
...
- `python -m Source.News.gdelt --days 730` (free, NO key) writes `Data/Processed_Data/gdelt_sentiment.csv`. Rate-limited; re-run to fill gaps, it resumes.
...
- The old NewsAPI+FinBERT path was REMOVED: ~30 days of history meant zero-filling 99% of the training sample, which invents a feature rather than adding one. GDELT reaches 2017 and supersedes it on every axis.
...
- `suite.auc_pvalue` is **TWO-SIDED by default**. One-sided is structurally blind to anti-skill: a model ranking backwards reports p~1.0, which reads as "no information" when the truth is "information, wrong sign". It also matters for honesty - the SIGN of any apparent edge is only learned from the test set, so deciding post-hoc that you would have inverted the signal is look-ahead, and both tails must cost multiple-testing budget.
- **The h=1 trap.** h=1 is the only zero-overlap horizon (eff n 675, not 33) and scores AUC 0.443, two-sided p 0.010 - the smallest in the family. It is NOT a finding: 0/20 survive Bonferroni/BH, and it decays across sub-periods (p 0.002 -> 0.405). Treating it as "pre-registered because it is structurally special" is post-hoc - large eff n mechanically produces the smallest p. A regression test asserts nothing is marked actionable unless its CI is strictly above 0.50.
